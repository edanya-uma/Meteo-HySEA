...
int main(int argc, char *argv[])
{
	...
	double defTime[MAX_FAULTS];
	//double *deformacionNivel0[MAX_FAULTS];
	float *presion0;	//guardo la presion inicial aqui
	//double *deformacionAcumNivel0;	//ESO QUITAR
	double f_coriolis;	//NUEVO
	...
	//err = cargarDatosProblema(fich_ent, nombre_bati, prefijo, &numNiveles, &okada_flag, &LON_C, &LAT_C, &DEPTH_C, &FAULT_L,
	//		&FAULT_W, &STRIKE, &DIP, &RAKE, &SLIP, &datosVolumenesNivel_1, &datosVolumenesNivel_2, &datosNivel, &numVolxNivel0,
	//		&numVolyNivel0, &numVolumenesNivel, &numEstadosDefDinamica, defTime, deformacionNivel0, &deformacionAcumNivel0,
	//		fich_def, &leer_fichero_puntos, &numPuntosGuardar, &posicionesVolumenesGuardado, &lonPuntos, &latPuntos, &Hmin,
	//		&borde_sup, &borde_inf, &borde_izq, &borde_der, &tam_spongeSup, &tam_spongeInf, &tam_spongeIzq, &tam_spongeDer,
	//		&tiempo_tot, &tiempoGuardarNetCDF,&tiempoGuardarSeries, &CFL, &mf0, &vmax, &epsilon_h, &hpos, &cvis, &L, &H, &Q, &T, &sinPhi);	//NUEVO (MODIFICADO)

	err = cargarDatosProblema(directorio, fich_ent, nombre_bati, prefijo, &numNiveles, &okada_flag, &LON_C, &LAT_C, &DEPTH_C, &FAULT_L,
			&FAULT_W, &STRIKE, &DIP, &RAKE, &SLIP, &datosVolumenesNivel_1, &datosVolumenesNivel_2, &datosNivel, &numVolxNivel0,
			&numVolyNivel0, &numVolumenesNivel, &numEstadosDefDinamica, defTime, &presion0,
			fich_def, &leer_fichero_puntos, &numPuntosGuardar, &posicionesVolumenesGuardado, &lonPuntos, &latPuntos, &Hmin,
			&borde_sup, &borde_inf, &borde_izq, &borde_der, &tam_spongeSup, &tam_spongeInf, &tam_spongeIzq, &tam_spongeDer,
			&tiempo_tot, &tiempoGuardarNetCDF,&tiempoGuardarSeries, &CFL, &mf0, &vmax, &epsilon_h, &hpos, &cvis, &L, &H, &Q, &T, &sinPhi);	//NUEVO (MODIFICADO)
	if (err > 0)
	...
	err = shallowWater(directorio, numNiveles, okada_flag, LON_C, LAT_C, DEPTH_C, FAULT_L, FAULT_W, STRIKE, DIP, RAKE, SLIP,
			datosVolumenesNivel_1, datosVolumenesNivel_2, datosNivel, numEstadosDefDinamica, defTime, presion0,
			fich_def, leer_fichero_puntos, numPuntosGuardar, posicionesVolumenesGuardado, lonPuntos, latPuntos,
			numVolxNivel0, numVolyNivel0, numVolumenesNivel, Hmin, (char *) nombre_bati.c_str(), prefijo, borde_sup,
			borde_inf, borde_izq, borde_der, tam_spongeSup, tam_spongeInf, tam_spongeIzq, tam_spongeDer, tiempo_tot,
			tiempoGuardarNetCDF, tiempoGuardarSeries, CFL, mf0, vmax, epsilon_h, hpos, cvis, L, H, Q, T, &tiempo_gpu, radio_tierra, sinPhi);	//NUEVO (MODIFICADO)
	if (err > 0) {
		if (err == 1)
