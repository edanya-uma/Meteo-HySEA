...
void truncarDeltaTParaSigDeformacion(int okada_flag, int fallaOkada, double tiempoActSubmalla, double *defTime,
		int indiceEstadoSigDefDim, double T, double *delta_T, bool *saltar_deformacion)
{
	if ((fallaOkada == 0) && (okada_flag == DYNAMIC_DEFORMATION)) {
		if ((tiempoActSubmalla + (*delta_T))*T > defTime[indiceEstadoSigDefDim]) {
			*delta_T = abs(defTime[indiceEstadoSigDefDim]/T - tiempoActSubmalla);	//NUEVO
			*saltar_deformacion = true;
		}
	}
}

void truncarDeltaTParaSigDeformacionSaltando(int okada_flag, int fallaOkada, double tiempoActSubmalla, double *defTime,
		int numEstadosDefDinamica, int indiceEstadoSigDefDim, double T, double *delta_T, bool *saltar_deformacion)
{
	*saltar_deformacion = false;
	if ((fallaOkada == 0) && (okada_flag == DYNAMIC_DEFORMATION)) {
		if (indiceEstadoSigDefDim+1 < numEstadosDefDinamica) {
			if ((tiempoActSubmalla + (*delta_T))*T > defTime[indiceEstadoSigDefDim+1]) {
				*delta_T = abs(defTime[indiceEstadoSigDefDim+1]/T - tiempoActSubmalla);	//NUEVO
				*saltar_deformacion = true;
			}
		}
	}
...
void liberarMemoria(int numNiveles, double2 *d_datosVolumenesNivel_1, double2 *d_datosVolumenesNivel_2,
		...
		int *d_posicionesVolumenesGuardado, double2 *d_datosVolumenesGuardado_1,
		double2 *d_datosVolumenesGuardado_2, int okada_flag, int numEstadosDefDinamica,
		float *d_presion0, float *d_presion1, double *d_original_bathy, double *d_sinPhi) //NUEVO
{
	//int i;
	...
	if (okada_flag == DYNAMIC_DEFORMATION) {
		cudaFree(d_original_bathy); //NUEVO 
		cudaFree(d_presion0); //NUEVO 
		cudaFree(d_presion1); //NUEVO 
...
double siguientePasoNivel0(double2 *d_datosVolumenesNivel0_1, double2 *d_datosVolumenesNivel0_2, tipoDatosSubmalla *d_datosNivel0,
		...
		dim3 blockGridVer2, dim3 blockGridHor1, dim3 blockGridHor2, dim3 threadBlockAri, dim3 blockGridEst, dim3 threadBlockEst, double radio_tierra, double *sinPhi, double T)	//NUEVO
{
	// PASO 1
	...
	procesarAristasVerNivel0Paso2GPU<<<blockGridVer1, threadBlockAri>>>(d_datosVolumenesNivel0_1, d_datosVolumenesNivel0_2,
		...
		delta_T, d_acumuladorNivel_1, d_acumuladorNivel_2, CFL, epsilon_h, hpos, cvis, 1, radio_tierra, sinPhi, T);	//NUEVO
	procesarAristasVerNivel0Paso2GPU<<<blockGridVer2, threadBlockAri>>>(d_datosVolumenesNivel0_1, d_datosVolumenesNivel0_2,
		d_datosNivel0->areaYCosPhi, d_datosNivel0->altoVolumenes, numVolxNivel0, numVolyNivel0, borde_izq, borde_der,
		delta_T, d_acumuladorNivel_1, d_acumuladorNivel_2, CFL, epsilon_h, hpos, cvis, 2, radio_tierra, sinPhi, T);	//NUEVO
...
extern "C" int shallowWater(string directorio, int numNiveles, int okada_flag, double LON_C, double LAT_C, double DEPTH_C, double FAULT_L,
			double FAULT_W, double STRIKE, double DIP, double RAKE, double SLIP, double2 *datosVolumenesNivel_1,
			double2 *datosVolumenesNivel_2, tipoDatosSubmalla datosNivel, int numEstadosDefDinamica, double *defTime,
			float *presion0, string fich_def, int leer_fichero_puntos, int numPuntosGuardar,
			...
			double epsilon_h, double hpos, double cvis, double L, double H, double Q, double T, double *tiempo, double radio_tierra, double *sinPhi)	//NUEVO
{
	//HE QUITADO DEL ARGUMENTO deformacionNivel0
	double2 *d_datosVolumenesNivel_1; 
	double *d_original_bathy;	//NUEVO aqui guardo la bathy original y en *d_datosVolumenesNivel_1.y la meto junto con la presion atmosferica para los calculos
	double *d_sinPhi;	//NUEVO
	...
	float *d_presion0;	//guardamos aqui la presion en instante n
	float *d_presion1;	//guardamos aqui la presion en instante n+1
	int indiceEstadoSigDefDim = 0;
	...
	cudaMalloc((void **)&(d_sinPhi), numVolyNivel0*sizeof(double));	//NUEVO
	...
	if (okada_flag == DYNAMIC_DEFORMATION) {
		cudaMalloc((void **)&d_original_bathy, tam_datosVolDoubleNivel); //NUEVO
		cudaMalloc((void **)&(d_presion0), (int64_t) numVolxNivel0*numVolyNivel0*sizeof(float));
		cudaMalloc((void **)&(d_presion1), (int64_t) numVolxNivel0*numVolyNivel0*sizeof(float));
		//for (i=0; i<numEstadosDefDinamica; i++)
		//	cudaMalloc((void **)&(d_deformacionNivel0[i]), (int64_t) numVolxNivel0*numVolyNivel0*sizeof(double));
	}
	...
	if (err == cudaErrorMemoryAllocation) {
		...
		cudaFree(d_presion0); //NUEVO
		cudaFree(d_presion1); //NUEVO
		//¿Aqui no habria que liberar tambien la memoria de d_deformacionNivel0?
		return 1;
	}
	...
	if (leer_fichero_puntos == 1) {
		...
		if (err == cudaErrorMemoryAllocation) {
			liberarMemoria(numNiveles, d_datosVolumenesNivel_1, d_datosVolumenesNivel_2, d_datosNivel,
				...
				okada_flag, numEstadosDefDinamica, d_presion0, d_presion1, d_original_bathy, d_sinPhi);
			return 1;
		}
	}
	...
	cudaMemcpy(d_sinPhi, sinPhi, numVolyNivel0*sizeof(double), cudaMemcpyHostToDevice);	//NUEVO
	...
	if (okada_flag == DYNAMIC_DEFORMATION) {
		int64_t tam2 = (int64_t) numVolxNivel0*numVolyNivel0*sizeof(float);
		copiarBathyDesdeComponenteY<<<blockGridEstNivel, threadBlockEst>>>(d_original_bathy, d_datosVolumenesNivel_1, numVolxNivel0, numVolyNivel0); //NUEVO copio en d_original_bathy la bathy original
		cudaMemcpy(d_presion0, presion0, tam2, cudaMemcpyHostToDevice);
		cudaMemcpy(d_presion1, presion0, tam2, cudaMemcpyHostToDevice);

		//inicialmente d_presion0 = d_presion1
		
		//cudaDeviceSynchronize(); // no se si hay que poner esto
		//for (i=0; i<numEstadosDefDinamica; i++) {
	...
	if (tiempoGuardarNetCDF >= 0.0) {
		...
		if (vec == NULL) {
			liberarMemoria(numNiveles, d_datosVolumenesNivel_1, d_datosVolumenesNivel_2, d_datosNivel,
				...
				d_datosVolumenesGuardado_2, okada_flag, numEstadosDefDinamica, d_presion0, d_presion1, d_original_bathy, d_sinPhi); //NUEVO
			return 2;
		}
		...
		iter = crearFicherosNC(nombre_bati, okada_flag, (char *) prefijo.c_str(), numVolxNivel0, numVolyNivel0,
					...
					FAULT_L, FAULT_W, STRIKE, DIP, RAKE, SLIP, fich_def, vec);
		if (iter == 1) {
			liberarMemoria(numNiveles, d_datosVolumenesNivel_1, d_datosVolumenesNivel_2, d_datosNivel,
				d_eta1MaximaNivel, d_deltaTVolumenesNivel, d_acumuladorNivel_1, d_acumuladorNivel_2,
				leer_fichero_puntos, d_posicionesVolumenesGuardado, d_datosVolumenesGuardado_1,
	...
	if (leer_fichero_puntos == 1) {
		...
		initTimeSeriesNC(nombre_bati, (char *) prefijo.c_str(), numPuntosGuardar, lonPuntos, latPuntos, tiempo_tot*T,
			CFL, epsilon_h*H, mf0_ini, vmax*Q/H, hpos*H, 1.0-cvis, borde_sup, borde_inf, borde_izq, borde_der,
			LON_C, LAT_C, DEPTH_C, FAULT_L, FAULT_W, STRIKE, DIP, RAKE, SLIP, okada_flag, fich_def);
		for (i=0; i<numPuntosGuardar; i++) {
			if (posicionesVolumenesGuardado[i] == -1) {
	...
	while (tiempoActSubmalla < tiempo_tot) {
		if (okada_flag == DYNAMIC_DEFORMATION){
			if (fallaOkada==0){
				copiarBathyHaciaComponenteY<<<blockGridEstNivel, threadBlockEst>>>(d_original_bathy, d_datosVolumenesNivel_1, numVolxNivel0, numVolyNivel0); //NUEVO copio en d_datosVolumenesNivel_1 la bathy original.
				// Esto de copiar la bathy hay que hacerlo mientras dure la deformacion dinamica. Cuando haya terminado la defDinamica, en d_datosVolumenesNivel_1.y deberia quedarse la bati original ya todo el rato.
				cudaDeviceSynchronize(); // no se si hay que poner esto
			}
		    else if (fallaOkada==1 && k==0){
				//aqui entra cuando ya estamos sin snapshots de presion. Solo tiene que entrar una vez para dejar copiada la bathy original en d_datosVolumenesNivel_1.y
				k+=1;
				copiarBathyHaciaComponenteY<<<blockGridEstNivel, threadBlockEst>>>(d_original_bathy, d_datosVolumenesNivel_1, numVolxNivel0, numVolyNivel0); //NUEVO copio en d_datosVolumenesNivel_1 la bathy original.
				cudaDeviceSynchronize(); // no se si hay que poner esto
			}
		...
		comprobarYAplicarDeformacionDinamica(d_datosVolumenesNivel_1, d_presion0, d_presion1, d_deltaTVolumenesNivel,
			tiempoAntSubmalla, tiempoActSubmalla, numVolxNivel0, numVolyNivel0, okada_flag, &fallaOkada, defTime,
			numEstadosDefDinamica, &indiceEstadoSigDefDim, blockGridEstNivel, threadBlockEst, T, presion0, directorio, fich_def, H, Q);	//NUEVO



		tiempoAntSubmalla = tiempoActSubmalla;
		deltaTNivel = siguientePasoNivel0(d_datosVolumenesNivel_1, d_datosVolumenesNivel_2, &d_datosNivel, d_acumuladorNivel_1,
						...
						blockGridEstNivel, threadBlockEst, radio_tierra, d_sinPhi, T);	//NUEVO

		if (! saltar_deformacion) {
			truncarDeltaTParaSigDeformacion(okada_flag, fallaOkada, tiempoActSubmalla, defTime,
				indiceEstadoSigDefDim, T, &deltaTNivel, &saltar_deformacion);
		}
		else {
			truncarDeltaTParaSigDeformacionSaltando(okada_flag, fallaOkada, tiempoActSubmalla, defTime,
				numEstadosDefDinamica, indiceEstadoSigDefDim, T, &deltaTNivel, &saltar_deformacion);
		}
	...
	*tiempo = sdkGetTimerValue(&timer)*0.001;
	...
	liberarMemoria(numNiveles, d_datosVolumenesNivel_1, d_datosVolumenesNivel_2, d_datosNivel,
		d_eta1MaximaNivel, d_deltaTVolumenesNivel, d_acumuladorNivel_1, d_acumuladorNivel_2,
		leer_fichero_puntos, d_posicionesVolumenesGuardado, d_datosVolumenesGuardado_1,
		d_datosVolumenesGuardado_2, okada_flag, numEstadosDefDinamica, d_presion0, d_presion1, d_original_bathy, d_sinPhi);
