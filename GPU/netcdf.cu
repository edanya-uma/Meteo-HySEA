...
void crearFicheroNC(double *lon_grid, double *lat_grid, double *lon, double *lat, char *nombre_bati, int okada_flag,
			...
			double RAKE, double SLIP, string fich_def, float *bati)
{
	...
	if (okada_flag == SEA_SURFACE_FROM_FILE) {
		...
		check_err(iret);
		sprintf(cadena, "file: %s", (char *) fich_def.c_str());
		iret = nc_put_att_text(ncid, NC_GLOBAL, "dynamic_deformation", strlen(cadena), cadena);
		check_err(iret);
	}
...
int crearFicherosNC(char *nombre_bati, int okada_flag, char *prefijo, int num_volx, int num_voly, double *lon_grid,
			...
			double DIP, double RAKE, double SLIP, string fich_def, float *bati)
{
	double *lon, *lat;
	int i;
	...
	crearFicheroNC(lon_grid, lat_grid, lon, lat, nombre_bati, okada_flag, prefijo, &ncid, &time_id, &eta_id, &ux_id,
		&uy_id, num_volx, num_voly, tiempo_tot, CFL, epsilon_h, mf0, vmax, hpos, cvis, borde_sup, borde_inf,
		borde_izq, borde_der, LON_C, LAT_C, DEPTH_C, FAULT_L, FAULT_W, STRIKE, DIP, RAKE, SLIP, fich_def, bati);
